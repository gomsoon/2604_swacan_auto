...
#### `alert_rules` lifecycle MVP 구현 방향

현재 `alert_rules`는 단순 threshold rule row에 가깝고, `rule_key`, `display_name`, `status`가 없다.  
지금까지 합의한 정책을 실제 구현으로 옮기려면, `alert_rules`를 `draft / published / deprecated` lifecycle을 가진 엔터티로 끌어올리는 것이 적절하다.
...
기존에 저장된 `alert_rules`는 migration 시점에 모두 `published`로 보는 것이 적절하다.

이유:
- 현재 운영 중인 rule을 실수로 편집 가능한 draft로 노출하지 않기 위함
- 기존 rule은 이미 운영 기준에 들어간 것으로 해석하는 편이 안전함

권장 backfill 방향:
- `status = 'published'`
- `rule_key = threshold.<state_type>.<metric_key>.legacy-<id>`
- `display_name`
  - `description`이 있으면 우선 사용
  - 없으면 `Legacy <state_type> <metric_key> threshold` 같은 deterministic 이름 사용
...
- `PATCH /api/admin/alert-rules/{id}`
  - `draft`일 때만 semantic field 수정 허용
  - `published` 이후에는 semantic field 수정 차단
  - 단, `is_enabled` 토글은 운영 action으로 계속 허용 가능
- `POST /api/admin/alert-rules/{id}/publish`
  - draft만 published로 전환
- `POST /api/admin/alert-rules/{id}/clone`
  - 새 draft 생성
  - `rule_key`는 `-2`, `-3` suffix 자동 제안
  - `display_name`은 ` (Copy)` suffix 기반 이름 자동 제안
  - `is_enabled`는 기본적으로 `false`로 시작하는 편이 안전함

`PATCH /api/admin/alert-rules/{id}`에서 field별 권장 정책:
- 항상 허용:
  - `is_enabled`
- `draft`에서만 허용:
  - `rule_key`
...
즉 MVP에서는 `display_name`과 `description`도 `published` 이후에는 freeze하는 편이 더 단순하고 안전하다.

##### Admin UI editable matrix

- `draft`
  - `rule_key`, `display_name`, selector, threshold, description 모두 편집 가능
- `published`
  - semantic field read-only
  - `is_enabled`만 토글 가능
  - 변경이 필요하면 `Clone as draft` 버튼으로 새 draft 생성
- `deprecated`
  - read-only

##### Preview / catalog 연결

preview와 candidate rule catalog에도 다음 필드를 포함하는 편이 적절하다.
- `rule_key`
- `display_name`
- `status`

이렇게 하면:
- 운영자는 `display_name`으로 candidate/winner rule을 이해
- 시스템은 `rule_key`로 안정적으로 참조
- preview와 archive/history가 같은 naming contract를 공유

##### 테스트 우선순위

MVP에서 먼저 닫아야 하는 테스트는 이 정도가 적절하다.
- 기존 rule backfill 후 모두 `published`
- `published` rule에서 semantic field patch 차단
- `published` rule에서 `is_enabled` patch 허용
- duplicate `rule_key` 차단
- invalid `rule_key` message
- clone 시 suffix 증가
- list / preview 응답에 `rule_key`, `display_name`, `status` 노출

##### Clone / publish 세부 정책

`clone`은 의미 변경이나 표현 조정이 필요한 경우의 기본 분기점으로 보는 것이 적절하다.

권장 기본 동작:
- 원본 rule을 기반으로 새 row 생성
- 새 row는 항상 `status = 'draft'`
- 새 row는 기본적으로 `is_enabled = 0`
- `rule_key`
  - 같은 prefix를 유지
  - 충돌 시 `-2`, `-3` suffix 증가
- `display_name`
  - 원본 이름을 유지하되 ` (Copy)` suffix 부여

이렇게 두면 복제 직후 rule이 실수로 운영 평가에 바로 섞이지 않고, 운영자가 draft에서 다시 의도를 다듬은 뒤 publish할 수 있다.

`publish`는 단순 status 전환이 아니라, rule identity를 고정하는 checkpoint로 보는 것이 적절하다.

권장 publish precondition:
- `rule_key` validation 통과
...
- `rule_key` freeze
- `display_name` freeze
- selector / threshold / description freeze
- 운영 action으로서 `is_enabled`만 토글 가능

즉 MVP의 핵심 원칙은:
- 초안 편집은 `draft`
- 운영 반영 기준점은 `published`
- 바꾸고 싶으면 `clone -> new draft`

##### PATCH editable matrix
...
- `draft`
  ...
  - threshold fields: editable
  - `description`: editable
  - `is_enabled`: editable
- `published`
  - `rule_key`: locked
  - `display_name`: locked
  - selector fields: locked
  - threshold fields: locked
  - `description`: locked
  - `is_enabled`: editable
- `deprecated`
  - 전체 read-only
