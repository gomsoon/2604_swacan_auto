...
### 4.1 MVP backend 배포 단위

- [필수] MVP backend 는 단일 Flask 웹서버 프로세스 하나에 모든 책임을 집중시키지 말고, 최소한 `web` 과 `worker` 의 논리적 분리를 가져야 한다.
- [필수] `web` 은 인증, 메타모델 및 model CRUD, 관리자 API, monitoring 조회 API, agent ingest 진입점, SSE 응답을 담당해야 한다.
- [필수] `worker` 는 latest state 갱신, grouped event 생성, stale 판정, retention 정리, debug payload 정리 같은 백그라운드 처리를 담당해야 한다.
- [필수] MVP 에서는 하나의 서버 내 별도 프로세스 또는 동등한 분리된 실행 단위로 `web` 과 `worker` 를 운영할 수 있어야 한다.

### 4.2 Queue 및 broker 정책
...
- [필수] latest state, grouped event, raw event, debug payload 정리는 동시에 여러 경로에서 무질서하게 쓰기보다 제어된 쓰기 흐름을 가져야 한다.
- [필수] 개발용 Flask 서버와 운영 배포 구조는 구분해야 하며, 운영 시에는 적절한 WSGI 서버 환경을 전제로 해야 한다.

### 4.4 실시간 갱신 구조

- [필수] monitoring 화면에 대한 실시간 갱신은 `SSE 우선 + snapshot polling 보정` 구조를 전제로 backend 내부 아키텍처를 설계해야 한다.
- [필수] SSE 연결 수 증가가 backend web 프로세스 자원을 장시간 점유할 수 있음을 고려해, MVP 에서는 과도한 양방향 실시간 기능을 피해야 한다.
- [필수] event storm 상황에서는 raw event 전체를 바로 push 하지 않고 grouped event 중심으로 frontend 로 전달해야 한다.

### 4.5 구조적 확장 기준

- [필수] MVP 구조는 단순해야 하지만, 이후 PostgreSQL 전환, 외부 broker 도입, worker 수평 분리를 막지 않는 방향이어야 한다.
- [필수] backend 내부 모듈은 최소한 `web API`, `ingest`, `state processor`, `event processor`, `admin/log`, `debug payload` 책임이 분리 가능하도록 설계해야 한다.
- [필수] backend 구조적 아키텍처 요구사항은 이후 ERD, API, 배포 설계의 기준으로 사용해야 한다.
...
## 9. Latest State 와 Event 관리 요구사항
...
### 9.4 반복 이벤트 그룹화와 event storm 완화
...
### 11.0 1차 관리자 화면 범위와 구현 순서

- [필수] MVP 이후 첫 번째 관리자 화면은 read-only 중심의 기본 운영 화면으로 시작해야 한다.
- [필수] 1차 관리자 화면은 최소한 system summary, ingest inbox 상태 요약, 최근 raw event, 최근 debug payload 조회 기능을 제공해야 한다.
- [필수] 1차 관리자 기능은 `admin API backend` 를 먼저 구현하고, 이후 frontend 화면이 해당 API 를 소비하는 순서로 개발해야 한다.
- [필수] 관리자 화면 1차 버전은 현재 backend 가 실제로 저장하고 있는 데이터에 기반해야 하며, 아직 수집하지 않는 정보는 화면 요구사항으로 과장해서는 안 된다.
...
- [필수] 1차 관리자 화면은 세션 레지스트리가 아직 없는 상태를 전제로, 실시간 접속 사용자 목록 대신 사용자 수, view 수, ingest 상태 같은 최소 운영 요약 정보를 우선 제공할 수 있어야 한다.
- [후속] 세션 레지스트리와 heartbeat 저장 구조가 도입되면, 관리자는 현재 접속한 사용자의 목록을 조회할 수 있어야 한다.
- [후속] 세션 레지스트리와 heartbeat 저장 구조가 도입되면, 관리자는 각 사용자의 현재 모드가 편집인지 관제인지 확인할 수 있어야 한다.
- [후속] 세션 레지스트리와 heartbeat 저장 구조가 도입되면, 관리자는 사용자가 어떤 view 를 열고 있는지 확인할 수 있어야 한다.
- [후속] 세션 기반 관리자 화면에는 마지막 활동 시각이 포함되어야 한다.

### 11.3 운영 로그 조회

- [필수] 1차 관리자 화면은 `ingest_inbox`, `raw_events`, `debug_payload_logs` 기반의 read-only 운영 조회 기능을 제공해야 한다.
- [필수] ingest inbox 조회는 최소한 `pending`, `processing`, `processed`, `failed` 상태 요약과 최근 실패 항목을 확인할 수 있어야 한다.
- [필수] 관리자는 최근 1주일 범위의 구조화된 로그를 조회할 수 있어야 한다.
- [필수] 로그는 최소한 `timestamp`, `severity`, `component`, `message`, `related object id` 로 필터링 가능해야 한다.
- [후속] 관리자는 agent 연결 문제, 저장 실패, 인증 실패, 메타모델 변경, 주요 사용자 작업 로그를 하나의 구조화된 운영 로그 화면에서 통합 조회할 수 있어야 한다.

### 11.4 Debug payload 조회
...
### 12.3 Debug mode 통신 payload 기록 정책
...
- MVP backend 는 `web` 과 `worker` 책임을 분리하고, 외부 broker 없이 SQLite 기반 inbox/work queue 구조로 시작해야 한다.
