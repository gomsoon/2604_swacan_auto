...
- 기본 skeleton은 들어갔지만, 현재는 alert별 action row를 저장하는 수준이다.
- 상태 변경 사유 분류, operator note append-only log, bulk action 이력, 화면 drill-down UX는 더 다듬을 여지가 있다.
- 동일 alert lifecycle을 운영 화면에서 `1 row`로 이해하기 쉽도록 archive 구조를 별도 설계할 필요가 있다.
...
### P1. Alert history 고도화

- 현재 상태:
  - skeleton 구현 완료
- 다음 목표:
  - `current alert`와 `archive history` 역할 분리
  - lifecycle 1건당 history 1 row 구조 정리
  - operator note append-only history
  - action source 세분화
  - resolution reason / reopen reason 분리
  - history drill-down UX 보강
- 기대 효과:
  - 감사 추적 강화
  - 운영자 간 인수인계 개선
...
1. `alert lifecycle redesign` 문서 기준으로 current/archive 분리 범위 정리
2. suppression 정책 정리
