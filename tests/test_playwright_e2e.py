...
def test_playwright_admin_page(page: Page, live_server) -> None:
    ...
    expect(page.locator("#alert-rule-preview-panel")).to_contain_text("blocked compound draft")
    expect(page.locator("#alert-rule-preview-panel")).to_contain_text("Publish readiness: compound threshold는 아직 publish를 지원하지 않습니다.")
    page.locator("#alert-rule-metric-key").fill("fd_count")
    page.locator("#alert-rule-critical-clause1-comparison").select_option("gte")
    ...
    expect(page.locator("#alert-rule-editor-status")).to_contain_text("Publish 차단")
    expect(page.locator("#alert-rule-editor-status")).to_contain_text("compound threshold는 아직 publish를 지원하지 않습니다.")
    expect(page.locator("#publish-current-alert-rule-button")).to_be_visible()
