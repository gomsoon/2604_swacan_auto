...
def test_playwright_admin_page(page: Page, live_server) -> None:
    ...
    expect(page.locator("#metamodel-containment-rule-form-mode")).to_contain_text("create")
    expect(page.locator("#metamodel-containment-parent-type-id option:checked")).to_contain_text("Physical Server (PhysicalServer)")
    expect(page.locator("#metamodel-containment-child-type-id option:checked")).to_contain_text("Worker Pool Updated (WorkerPool)")
    ...
    page.locator('#metamodel-workspace-canvas [data-workspace-kind="semantic_type"]').filter(has_text="Worker Pool Updated").click()
    expect(page.locator("#metamodel-association-form-mode")).to_contain_text("create")
    expect(page.locator("#metamodel-association-source-type-id option:checked")).to_contain_text("Monitoring Agent (MonitoringAgent)")
    expect(page.locator("#metamodel-association-target-type-id option:checked")).to_contain_text("Worker Pool Updated (WorkerPool)")
    expect(page.locator("#metamodel-workspace-mode-status")).to_contain_text("선택 모드")
